...
public class IncrementRecordApplier extends AbstractRecordApplier {
    ...
    protected void applyOneByOne(List<IncrementRecord> incRecords, JdbcTemplate jdbcTemplate) {
        for (final IncrementRecord incRecord : incRecords) {
            ...
            jdbcTemplate.execute(applierSql, (PreparedStatementCallback) ps -> {
                ...
                if (!(sourceDbType.isSqlServer() && incRecord.getOpType() == IncrementOpType.D) && !incRecord.isSkipCheckColumnsCount()) {
    ...
    protected void processMissColumn(final IncrementRecord incRecord, final Map<String, Integer> indexes) {
        // 如果数量不同，则认为缺少主键
        List<String> allNames = new ArrayList<>(indexes.keySet());
        for (ColumnValue cv : incRecord.getColumns()) {
            Integer index = getIndex(indexes, cv, true);
            if (index != null) {
