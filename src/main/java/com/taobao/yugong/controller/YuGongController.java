...
import static com.taobao.yugong.common.utils.YuGongToStringStyle.DEFAULT_DATE_TIME_FORMATTER;
        ...
        if (runMode == RunMode.FULL || runMode == RunMode.CHECK) {
            ...
            if (sourceDbType == DbType.ORACLE) {
                ...
                if (null == dateStartString && positioner != null && positioner.getLatest() != null) {
                    dateStartString = ((IdPosition) positioner.getLatest()).getUpdateTimeString();
                }
...
//            canalServerPort);
    ...
    private boolean isOnlyPkIsNumber(Table table) {
        if (table.getPrimaryKeys() != null && table.getPrimaryKeys().size() == 1) {
            return YuGongUtils.isNumberType(table.getPrimaryKeys().get(0).getType());
        }
